    ...
    pub fn new(data: &'a [u8]) -> Result<Self, String> {
        if &data[..4] == b"$BHD" || &data[..4] == b"$BL2" {
            let header: &DirectoryHeader = plain::from_bytes(&data)
                .map_err(|err| format!("BIOS directory header invalid: {:?}", err))?;
            ...
            return Ok(Self {
                ...
                .map_err(|err| format!("BIOS directory entries invalid: {:?}", err))?,
            });
        }
    ...
    pub fn new(data: &'a [u8]) -> Result<Self, String> {
        if &data[..4] == b"2BHD" {
            let header: &ComboDirectoryHeader = plain::from_bytes(&data)
                .map_err(|err| format!("BIOS combo header invalid: {:?}", err))?;
            ...
            return Ok(Self {
                ...
                .map_err(|err| format!("BIOS combo entries invalid: {:?}", err))?,
            });
        }
